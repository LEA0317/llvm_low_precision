...
TargetInfo::TargetInfo(const llvm::Triple &T) : TargetOpts(), Triple(T) {
  ...
  Float4Format = &llvm::APFloat::IEEEfloat4();
  Float8Format = &llvm::APFloat::IEEEfloat8();
...
const char *TargetInfo::getTypeFormatModifier(IntType T) {
  ...
  default: llvm_unreachable("not an integer!");
  case SignedInt4:
  case UnsignedInt4:     return "hhh"; // FIXME
  case SignedChar:
  ...
  case SignedInt256:
  case UnsignedInt256:   return "lll"; // FIXME
  }
}
...
TargetInfo::RealType TargetInfo::getRealTypeByWidth(unsigned BitWidth) const {
  if (getFloat4Width() == BitWidth)
    return Float4;
  if (getFloat8Width() == BitWidth)
    return Float8;
