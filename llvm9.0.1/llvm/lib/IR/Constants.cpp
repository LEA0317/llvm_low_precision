...
Constant *Constant::getNullValue(Type *Ty) {
  ...
  case Type::Float4TyID:
    return ConstantFP::get(Ty->getContext(),
                           APFloat::getZero(APFloat::IEEEfloat4()));
  case Type::Float8TyID:
    return ConstantFP::get(Ty->getContext(),
                           APFloat::getZero(APFloat::IEEEfloat8()));
  case Type::HalfTyID:
    return ConstantFP::get(Ty->getContext(),
...
static const fltSemantics *TypeToFloatSemantics(Type *Ty) {
  if (Ty->isFloat4Ty())
    return &APFloat::IEEEfloat4();
  if (Ty->isFloat8Ty())
    return &APFloat::IEEEfloat8();
  if (Ty->isHalfTy())
...
ConstantFP* ConstantFP::get(LLVMContext &Context, const APFloat& V) {
  ...
  if (!Slot) {
    Type *Ty;
    if (&V.getSemantics() == &APFloat::IEEEfloat4())
      Ty = Type::getFloat4Ty(Context);
    else if (&V.getSemantics() == &APFloat::IEEEfloat8())
      Ty = Type::getFloat8Ty(Context);
...
bool ConstantFP::isValueValidForType(Type *Ty, const APFloat& Val) {
  ...
  case Type::Float4TyID: {
    if (&Val2.getSemantics() == &APFloat::IEEEfloat4())
      return true;
    Val2.convert(APFloat::IEEEfloat4(), APFloat::rmNearestTiesToEven, &losesInfo);
    return !losesInfo;
  }
  case Type::Float8TyID: {
    if (&Val2.getSemantics() == &APFloat::IEEEfloat8())
      return true;
    Val2.convert(APFloat::IEEEfloat8(), APFloat::rmNearestTiesToEven, &losesInfo);
    return !losesInfo;
  }
  case Type::HalfTyID: {
  ...
  case Type::FloatTyID: {
    if (&Val2.getSemantics() == &APFloat::IEEEhalf() ||
	&Val2.getSemantics() == &APFloat::IEEEfloat4() ||
	&Val2.getSemantics() == &APFloat::IEEEfloat8() ||
  ...
  case Type::X86_FP80TyID:
    return &Val2.getSemantics() == &APFloat::IEEEfloat4() ||
           &Val2.getSemantics() == &APFloat::IEEEfloat8() ||
           &Val2.getSemantics() == &APFloat::IEEEhalf() ||
           &Val2.getSemantics() == &APFloat::IEEEsingle() ||
           &Val2.getSemantics() == &APFloat::IEEEdouble() ||
...
Constant *ConstantDataArray::getFP(LLVMContext         &Context,
  ...
  Type *Ty = ArrayType::get(Type::getFloat4Ty(Context), Elts.size());
  const char *Data = reinterpret_cast<const char *>(Elts.data());
  return getImpl(StringRef(Data, Elts.size()), Ty); // FIXME(konda)
}
Constant *ConstantDataArray::getFP(LLVMContext      &Context,
                                   ArrayRef<uint8_t> Elts) {
  Type *Ty = ArrayType::get(Type::getFloat8Ty(Context), Elts.size());
  const char *Data = reinterpret_cast<const char *>(Elts.data());
  return getImpl(StringRef(Data, Elts.size()), Ty);
}
Constant *ConstantDataArray::getFP(LLVMContext &Context,
...
Constant *ConstantDataVector::getFP(LLVMContext         &Context,
                                    ArrayRef<__int128_t> Elts) {
  Type *Ty = VectorType::get(Type::getFloat4Ty(Context), Elts.size());
  const char *Data = reinterpret_cast<const char *>(Elts.data());
  return getImpl(StringRef(Data, Elts.size()), Ty); // FIXME(konda)
}
Constant *ConstantDataVector::getFP(LLVMContext      &Context,
                                    ArrayRef<uint8_t> Elts) {
  Type *Ty = VectorType::get(Type::getFloat8Ty(Context), Elts.size());
  const char *Data = reinterpret_cast<const char *>(Elts.data());
  return getImpl(StringRef(Data, Elts.size()), Ty);
}
Constant *ConstantDataVector::getFP(LLVMContext &Context,
...
APFloat ConstantDataSequential::getElementAsAPFloat(unsigned Elt) const {
  ...
  case Type::Float4TyID: {
    ...
    return APFloat(APFloat::IEEEfloat4(), APInt(4, EltVal));
  }
  case Type::Float8TyID: {
    auto EltVal = *reinterpret_cast<const uint8_t *>(EltPtr);
    return APFloat(APFloat::IEEEfloat8(), APInt(8, EltVal));
  }
  case Type::HalfTyID: {
    auto EltVal = *reinterpret_cast<const uint16_t *>(EltPtr);
