...
public:
  ...
  bool isFloatingPoint() const {
    return getKind() >= Float4 && getKind() <= Float128;
  }
...
inline bool Type::isFloat4Type() const {
  if (const auto *BT = dyn_cast<BuiltinType>(CanonicalType))
    return BT->getKind() == BuiltinType::Float4;
  return false;
}

inline bool Type::isFloat8Type() const {
  if (const auto *BT = dyn_cast<BuiltinType>(CanonicalType))
    return BT->getKind() == BuiltinType::Float8;
  return false;
}
