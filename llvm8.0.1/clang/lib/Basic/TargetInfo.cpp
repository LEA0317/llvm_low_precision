...
TargetInfo::TargetInfo(const llvm::Triple &T) : TargetOpts(), Triple(T) {
  ...
  Fixed4Width = 4; // FIXME?(konda)
  Fixed4Align = 8; // FIXME?(konda)
  Fixed8Width = 8; // FIXME?(konda)
  Fixed8Align = 8; // FIXME?(konda)
  HalfWidth = 16;
  HalfAlign = 16;
  FloatWidth = 32;
  FloatAlign = 32;
...
const char *TargetInfo::getTypeName(IntType T) {
  ...
  case SignedInt256:     return "signed int256";
...
const char *TargetInfo::getTypeFormatModifier(IntType T) {
  ...
  default: llvm_unreachable("not an integer!");
  case SignedInt4:
  case UnsignedInt4:     return "hhh"; // FIXME(konda)
  case SignedChar:
  ...
  case SignedInt256:
  case UnsignedInt256:   return "lll"; // FIXME(konda)
  }
}
...
unsigned TargetInfo::getTypeWidth(IntType T) const {
  ...
  default: llvm_unreachable("not an integer!");
  case SignedInt4:
  case UnsignedInt4:     return getInt4Width();
  case SignedInt256:
...
TargetInfo::RealType TargetInfo::getRealTypeByWidth(unsigned BitWidth) const {
  if (getFixed4Width() == BitWidth)
    return Fixed4;
  if (getFixed8Width() == BitWidth)
    return Fixed8;
