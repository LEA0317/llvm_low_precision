...
Constant *FoldReinterpretLoadFromConstPtr(Constant *C, Type *LoadTy,
  ...
  if (!IntType) {
    ...
    // an actual new load.
    Type *MapTy;
    if (LoadTy->isFloat4Ty()) {
      MapTy = Type::getInt4Ty(C->getContext());
    } else if (LoadTy->isFloat8Ty()) {
...
Constant *GetConstantFoldFPValue(double V, Type *Ty) {
  if (Ty->isFloat4Ty()) {
    APFloat APF(V);
    bool unused;
    APF.convert(APFloat::IEEEfloat4(), APFloat::rmNearestTiesToEven, &unused);
    return ConstantFP::get(Ty->getContext(), APF);
  }
  if (Ty->isFloat8Ty()) {
    APFloat APF(V);
    bool unused;
    APF.convert(APFloat::IEEEfloat8(), APFloat::rmNearestTiesToEven, &unused);
    return ConstantFP::get(Ty->getContext(), APF);
  }
...
static Constant *ConstantFoldScalarCall1(StringRef Name,
  ...
  if (auto *Op = dyn_cast<ConstantFP>(Operands[0])) {
    ...
    if (!Ty->isHalfTy() && !Ty->isFloatTy() && !Ty->isDoubleTy() && !Ty->isFloat4Ty() && !Ty->isFloat8Ty())
      return nullptr;
  ...
  if (Call->getNumArgOperands() == 1) {
    if (ConstantFP *OpC = dyn_cast<ConstantFP>(Call->getArgOperand(0))) {
      ...
      case LibFunc_tanf: {
        ...
        Type *Ty = OpC->getType();
        if (Ty->isDoubleTy() || Ty->isFloatTy() || Ty->isHalfTy() || Ty->isFloat4Ty() || Ty->isFloat8Ty()) {
  ...
  if (Call->getNumArgOperands() == 2) {
    ...
    if (Op0C && Op1C) {
      ...
      case LibFunc_powf: {
        ...
        if (Ty->isDoubleTy() || Ty->isFloatTy() || Ty->isHalfTy() || Ty->isFloat4Ty() ||
 Ty->isFloat8Ty()) {
          if (Ty == Op1C->getType()) {
