...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT,
  ...
  if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(Operand)) {
    ...
    case ISD::BITCAST:
      if (VT == MVT::f4 && C->getValueType(0) == MVT::i4)
        return getConstantFP(APFloat(APFloat::IEEEfloat4(), Val), DL, VT);
      if (VT == MVT::f8 && C->getValueType(0) == MVT::i8)
	return getConstantFP(APFloat(APFloat::IEEEfloat8(), Val), DL, VT);
